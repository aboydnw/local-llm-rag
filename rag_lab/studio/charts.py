...
def _row_labels(records: list[RunRecord]) -> dict[str, str]:
    """Map each run_id to a display label, disambiguating names that collide."""
    counts = Counter(r.name for r in records)
    return {
        r.run_id: f"{r.name} ({r.run_id[:6]})" if counts[r.name] > 1 else r.name
        for r in records
    }
