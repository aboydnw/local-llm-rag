...
def _render_direct_knobs(cfg: Config) -> None:
    ...
    cfg.retriever.reranker = st.radio(
        "Reranker",
        rerankers,
        index=rerankers.index(cfg.retriever.reranker),
    ...
    if cfg.retriever.reranker == "llm":
        cfg.retriever.rerank_candidates = st.number_input(
            "Rerank candidates",
            min_value=1,
            value=cfg.retriever.rerank_candidates,
...
def render(session, page_key: str, include_corpus: bool = True) -> Config:
    ...
    with st.expander(
        "Experiment settings", expanded=config_logic.config_expanded(session, page_key)
    ):
